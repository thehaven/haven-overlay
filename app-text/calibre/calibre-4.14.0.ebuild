...
SRC_URI="https://github.com/kovidgoyal/${PN}/archive/v${PV}.tar.gz"
...
src_compile() {
	...
	CALIBRE_PY3_PORT=1 "${PYTHON}" setup.py bootstrap || die
	PATH=${T}:${PATH} PYTHONPATH=${S}/src${PYTHONPATH:+:}${PYTHONPATH} \
	CALIBRE_PY3_PORT=1 "${PYTHON}" setup.py build || die
	PATH=${T}:${PATH} PYTHONPATH=${S}/src${PYTHONPATH:+:}${PYTHONPATH} \
	CALIBRE_PY3_PORT=1 "${PYTHON}" setup.py iso3166 || die
	PATH=${T}:${PATH} PYTHONPATH=${S}/src${PYTHONPATH:+:}${PYTHONPATH} \
	CALIBRE_PY3_PORT=1 "${PYTHON}" setup.py iso639 || die
}
...
src_install() {
	...
	PATH=${T}:${PATH} PYTHONPATH=${S}/src${PYTHONPATH:+:}${PYTHONPATH} \
	CALIBRE_PY3_PORT=1 "${PYTHON}" setup.py install \
		--root="${D}" \
		--prefix="${EPREFIX}/usr" \
