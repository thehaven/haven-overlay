...
DESCRIPTION="Push notifications to your phone or desktop via PUT/POST"
...
SRC_URI="https://github.com/binwiederhier/ntfy/archive/refs/tags/v${PV}.tar.gz
	${EGO_SUM_SRC_URI}"
...
python_check_deps() {
	python_has_version "dev-python/mkdocs-material[${PYTHON_USEDEP}]" &&
	python_has_version "dev-python/mkdocs-minify-plugin[${PYTHON_USEDEP}]"
}
...
src_compile() {
	...
	# Web UI (vite build; embedded into the server binary below)
	pushd "${S}/web" >/dev/null || die
	npm ci --no-audit --no-fund --no-progress || die
	npm run build || die
	mv build/index.html build/app.html || die
	rm -rf ../server/site || die
	mv build ../server/site || die
	popd >/dev/null || die

	# Documentation (mkdocs; embedded into the server binary below)
	"${EPYTHON}" -m mkdocs build || die

	# Server / client binary
	ego build -tags sqlite_omit_load_extension,osusergo,netgo \
		-ldflags "-s -w -X main.version=${PV} -X main.commit=unknown -X main.date=$(date +%s)" \
		-o ntfy . || die
}
...
src_install() {
	...
	if use systemd; then
		...
		systemd_douserunit "${FILESDIR}/ntfy-client-user.service"
	fi
	doinitd "${FILESDIR}/ntfy"

	insinto /etc/ntfy
	doins server/server.yml client/client.yml

	keepdir /etc/ntfy
	keepdir /var/cache/ntfy
	fowners ntfy:ntfy /var/cache/ntfy
	fperms 0750 /var/cache/ntfy
}

pkg_postinst() {
	elog "ntfy ${PV} installed."
	elog ""
	elog "Quick start:"
	elog "  ntfy serve  # run the server with defaults"
	elog ""
	elog "Configuration:"
	elog "  /etc/ntfy/server.yml  (all options commented = defaults)"
	elog "  Docs: https://docs.ntfy.sh/"
	elog ""
	elog "The server runs as user/group ntfy and caches attachments in /var/cache/ntfy."
	if use systemd; then
		elog ""
		elog "Start the service:"
		elog "  systemctl enable --now ntfy"
	fi
}
