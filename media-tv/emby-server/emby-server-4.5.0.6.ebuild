...
SRC_URI="https://github.com/MediaBrowser/Emby.Releases/archive/${PV}.tar.gz -> ${P}.tar.gz"
SLOT="0"
LICENSE="GPL-2"
IUSE=""
...
src_unpack() {
        unpack ${A}
        mv Emby-${PV/_beta/} emby-server-${PV/_beta/}
}
...
src_compile() {
	...
	xbuild /p:Configuration="Release Mono" /p:Platform="Any CPU" /t:clean MediaBrowser.sln || die "xbuild clean failed"
	xbuild /p:Configuration="Release Mono" /p:Platform="Any CPU" /t:build MediaBrowser.sln || die "xbuild build failed"
}
