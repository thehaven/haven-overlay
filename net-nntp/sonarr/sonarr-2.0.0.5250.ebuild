...
src_compile() {
    npm install
	#MONO_IOMAP=case xbuild ${S}/src/NzbDrone.sln
    #${S}/build.sh
    einfo "updating root certificates for mono certificate store"
    cert-sync --user /etc/ssl/certs/ca-certificates.crt
    einfo "now actually compile"
    export MONO_IOMAP=case
	xbuild /p:Configuration="Release" /p:Platform="x86" /t:Clean src/NzbDrone.sln || die "xbuild clean failed"
    mono tools/nuget/nuget.exe restore src/NzbDrone.sln
	xbuild /p:Configuration="Release" /p:Platform="x86" /t:Build /p:AllowedReferenceRelatedFileExtensions=.pdb src/NzbDrone.sln || die "xbuild build failed"
