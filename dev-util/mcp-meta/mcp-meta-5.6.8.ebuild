...
IUSE="ai-compressor alertmanager analyzer aws brave-search chrome-devtools-mcp composio context7 cortex desktop-commander docfork docker fetch filesystem forge gemini-deepsearch git github github-mcp-server gitlab gitmcp grafana headroom jc karakeep kubernetes librenms linkedin markdownlint mediawiki mem0 memory mesh network-tools obsidian opentofu pagerduty paperclip pass perplexity perplexity-ask perplexity-community playwright plur postgres powerpoint pytest ripgrep sentry serena sequential-thinking skillspector slack stele task-manager terraform time tree-sitter +wcgw wikipedia"
...
RDEPEND="
	...
	chrome-devtools-mcp? ( dev-util/chrome-devtools-mcp )
	composio? ( dev-util/composio-mcp )
	context7? ( dev-util/mcp-server-context7 )
	cortex? ( app-misc/cortex )
	...
	filesystem? ( dev-util/mcp-server-filesystem )
	...
	paperclip? ( app-misc/paperclip-mcp )
	pass? ( dev-util/mcp-server-pass )
	perplexity? ( dev-util/mcp-server-perplexity )
	perplexity-ask? ( dev-util/mcp-server-perplexity-ask )
	perplexity-community? ( dev-util/mcp-server-perplexity-community )
	playwright? ( dev-util/mcp-server-playwright )
	plur? ( app-misc/plur-mcp )
	postgres? ( dev-util/mcp-postgres )
