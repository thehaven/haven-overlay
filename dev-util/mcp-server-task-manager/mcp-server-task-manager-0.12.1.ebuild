...
SRC_URI="https://registry.npmjs.org/@blizzy/mcp-task-manager/-/mcp-task-manager-${PV}.tgz"
S="${WORKDIR}/package"
...
pkg_postinst() {
	...
	elog "    \"${PN}\": {"
	elog "      \"command\": \"/usr/bin/mcp-task-manager\","
	elog "      \"args\": [\"stdio\"]"
	elog "    }"
	elog ""
	...
	elog "    \"${PN}\": {"
	elog "      \"type\": \"local\","
	elog "      \"command\": [\"/usr/bin/mcp-task-manager\", \"stdio\"],"
	elog "      \"enabled\": true"
	elog "    }"
}
