...
# nvidia-cuda-runtime-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUDA RUNTIME
...
DESCRIPTION="NVIDIA CUDA Runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cuda-runtime-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/bc/46/a92db19b8309581092a3add7e6fceb4c301a3fd233969856a8cbf042cd3c/nvidia_cuda_runtime_cu12-12.9.79-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cuda_runtime"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cuda_runtime_cu12-12.9.79.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cuda-runtime-cu12 installs the NVIDIA CUDA RUNTIME shared libraries into"
	elog "python's site-packages under nvidia/cuda_runtime/lib/."
	elog ""
	...
	elog "nvidia/cuda_runtime/lib/ directory for the active Python interpreter, or"
