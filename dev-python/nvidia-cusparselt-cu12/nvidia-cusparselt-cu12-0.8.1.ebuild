...
# nvidia-cusparselt-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUSPARSELT
...
DESCRIPTION="NVIDIA CUDA cuSPARSELt runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cusparselt-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/bb/14/e46964290aa587cb9fb7df20efdc60528ddd00d291ccffec47617fb06ca3/nvidia_cusparselt_cu12-0.8.1-py3-none-manylinux2014_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cusparselt"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cusparselt_cu12-0.8.1.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cusparselt-cu12 installs the NVIDIA CUSPARSELT shared libraries into"
	elog "python's site-packages under nvidia/cusparselt/lib/."
	elog ""
	...
	elog "nvidia/cusparselt/lib/ directory for the active Python interpreter, or"
