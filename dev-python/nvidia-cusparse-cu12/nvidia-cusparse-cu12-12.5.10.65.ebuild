...
# nvidia-cusparse-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUSPARSE
...
DESCRIPTION="NVIDIA CUDA cuSPARSE runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cusparse-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/12/46/b0fd4b04f86577921feb97d8e2cf028afe04f614d17fb5013de9282c9216/nvidia_cusparse_cu12-12.5.10.65-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cusparse"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cusparse_cu12-12.5.10.65.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cusparse-cu12 installs the NVIDIA CUSPARSE shared libraries into"
	elog "python's site-packages under nvidia/cusparse/lib/."
	elog ""
	...
	elog "nvidia/cusparse/lib/ directory for the active Python interpreter, or"
