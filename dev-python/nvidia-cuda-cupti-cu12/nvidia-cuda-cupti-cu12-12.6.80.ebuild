...
# nvidia-cuda-cupti-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUDA CUPTI
...
DESCRIPTION="NVIDIA CUDA CUPTI runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cuda-cupti-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/49/60/7b6497946d74bcf1de852a21824d63baad12cd417db4195fc1bfe59db953/nvidia_cuda_cupti_cu12-${PV}-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cuda_cupti"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cuda_cupti_cu12-12.6.80.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cuda-cupti-cu12 installs the NVIDIA CUDA CUPTI shared libraries into"
	elog "python's site-packages under nvidia/cuda_cupti/lib/."
	elog ""
	...
	elog "nvidia/cuda_cupti/lib/ directory for the active Python interpreter, or"
