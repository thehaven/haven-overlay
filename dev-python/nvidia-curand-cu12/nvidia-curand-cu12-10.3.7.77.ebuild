...
# nvidia-curand-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CURAND
...
DESCRIPTION="NVIDIA CUDA cuRAND runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-curand-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/73/1b/44a01c4e70933637c93e6e1a8063d1e998b50213a6b65ac5a9169c47e98e/nvidia_curand_cu12-${PV}-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/curand"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_curand_cu12-10.3.7.77.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-curand-cu12 installs the NVIDIA CURAND shared libraries into"
	elog "python's site-packages under nvidia/curand/lib/."
	elog ""
	...
	elog "nvidia/curand/lib/ directory for the active Python interpreter, or"
