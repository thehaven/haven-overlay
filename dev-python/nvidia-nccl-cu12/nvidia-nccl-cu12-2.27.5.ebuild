...
# nvidia-nccl-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA NCCL
...
DESCRIPTION="NVIDIA NCCL collective communications runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-nccl-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/6e/89/f7a07dc961b60645dbbf42e80f2bc85ade7feb9a491b11a1e973aa00071f/nvidia_nccl_cu12-${PV}-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/nccl"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_nccl_cu12-2.27.5.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-nccl-cu12 installs the NVIDIA NCCL shared libraries into"
	elog "python's site-packages under nvidia/nccl/lib/."
	elog ""
	...
	elog "nvidia/nccl/lib/ directory for the active Python interpreter, or"
