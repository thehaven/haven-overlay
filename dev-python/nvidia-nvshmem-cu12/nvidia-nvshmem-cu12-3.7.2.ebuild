...
# nvidia-nvshmem-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA NVSHMEM
...
DESCRIPTION="NVIDIA NVSHMEM runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-nvshmem-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/63/03/cde130e3ff706784f9efd47204f299b15113ea08203e98a556603dac245d/nvidia_nvshmem_cu12-3.7.2-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/nvshmem"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_nvshmem_cu12-3.7.2.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-nvshmem-cu12 installs the NVIDIA NVSHMEM shared libraries into"
	elog "python's site-packages under nvidia/nvshmem/lib/."
	elog ""
	...
	elog "nvidia/nvshmem/lib/ directory for the active Python interpreter, or"
