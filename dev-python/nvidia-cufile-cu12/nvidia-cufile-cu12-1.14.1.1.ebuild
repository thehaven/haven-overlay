...
# nvidia-cufile-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUFILE
...
DESCRIPTION="NVIDIA cuFile runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cufile-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/ad/28/b960e06d705a440c030edd84e16888ee14c743390bdb2a6368e92ffe8ef8/nvidia_cufile_cu12-1.14.1.1-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cufile"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cufile_cu12-1.14.1.1.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cufile-cu12 installs the NVIDIA CUFILE shared libraries into"
	elog "python's site-packages under nvidia/cufile/lib/."
	elog ""
	...
	elog "nvidia/cufile/lib/ directory for the active Python interpreter, or"
