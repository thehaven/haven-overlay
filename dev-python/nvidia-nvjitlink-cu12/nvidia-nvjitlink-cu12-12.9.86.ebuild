...
# nvidia-nvjitlink-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA NVJITLINK
...
DESCRIPTION="NVIDIA CUDA NVJITLink runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-nvjitlink-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/46/0c/c75bbfb967457a0b7670b8ad267bfc4fffdf341c074e0a80db06c24ccfd4/nvidia_nvjitlink_cu12-12.9.86-py3-none-manylinux2010_x86_64.manylinux_2_12_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/nvjitlink"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_nvjitlink_cu12-12.9.86.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-nvjitlink-cu12 installs the NVIDIA NVJITLINK shared libraries into"
	elog "python's site-packages under nvidia/nvjitlink/lib/."
	elog ""
	...
	elog "nvidia/nvjitlink/lib/ directory for the active Python interpreter, or"
