...
# nvidia-cufft-cu12 — wheel-only install pattern
# ------------------------------------------------------------------
# Upstream ships only a manylinux x86_64 wheel (no sdist). The wheel
# is python-version-agnostic (py3-none-any) and contains NVIDIA CUFFT
...
DESCRIPTION="NVIDIA CUDA cuFFT runtime libraries (Python wheel)"
HOMEPAGE="https://pypi.org/project/nvidia-cufft-cu12/"
WHEEL_URL="https://files.pythonhosted.org/packages/95/f4/61e6996dd20481ee834f57a8e9dca28b1869366a135e0d42e2aa8493bdd4/nvidia_cufft_cu12-11.4.1.4-py3-none-manylinux2014_x86_64.manylinux_2_17_x86_64.whl"
...
python_install() {
	...
	doins -r "${WORKDIR}/nvidia/cufft"
	if [[ ! -e "${sitedir}/nvidia/__init__.py" ]]; then
		insinto "${sitedir}/nvidia"
		doins "${WORKDIR}/nvidia/__init__.py"
	fi
	insinto "${sitedir}"
	doins -r "${WORKDIR}/nvidia_cufft_cu12-11.4.1.4.dist-info"
}
...
pkg_postinst() {
	elog "nvidia-cufft-cu12 installs the NVIDIA CUFFT shared libraries into"
	elog "python's site-packages under nvidia/cufft/lib/."
	elog ""
	...
	elog "nvidia/cufft/lib/ directory for the active Python interpreter, or"
